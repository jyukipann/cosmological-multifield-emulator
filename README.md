...
## 手法の改良（SANの導入）
よく知られている通り、GANは学習における安定性が乏しい。
そのため、学習安定化のためのLossやモデル設計、その他tipsなどが広く研究されてきた。
しかし、理論的に安定化が保証されているものは少ない。

Slicing Adversarial Network (SAN) [ICLR 2024]はヒューリスティックに頼った解決策ではなく理論に基づいた手法である。
本プロジェクトでも採用することとした。

### 方針 - GANのSAN-ification -
#### 識別器の微調整
画像の入力から線形層の直前までを１つの特徴抽出器（h(x)）として、線形層を分類器（ω）として切り出す（スライスする）。
線形層はそのパラメータを正規化した形で持つ。バイアスも不要。
これら２つのユニットに対してそれぞれ別の損失関数を適用する。損失関数は後述する。

識別器が持つべき性質としてinjectivity（単射性）がある。
これは、画像空間で異なる画像であれば、潜在空間内でも異なる座標に投射される性質である。

単射性は、それが損なわれるような操作（層）を含まなければ実現できる。
単射性が損なわれる操作として例えば、ReLU、次元削減などが上げられる。

#### 損失関数
公式チュートリアルでは、h(x)にHinge Loss、ωにWasserstein Lossを適用していた。

本プロジェクトでは、再構成Lossなども含めて学習を進めるため、SANの手法の意図することを阻害しないように実装をすすめる必要がある。
...
```bash
nohup python src/train_san.py > train_log.out &
```
...
### 2024 06 17 SANに変更後の学習
できてほしい。lossについて理解が甘いので、うまく実装できているかわからない。
...
- [SAN: Inducing Metrizability of GAN with Discriminative Normalized Linear Layer](https://arxiv.org/html/2301.12811v4)