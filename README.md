...
### 識別器の入力にノイズを加える
ガウシアンノイズを識別器の入力に与えることによって識別器の問題の難易度を上げて，生成器が学習する時間を稼ぐ．
ノイズの加え方を段階的に減らしていく，Sigmoid関数をy軸反転させた上で，
...
### 前実験
pytorchのDCGANチュートリアルのプログラムに本プロジェクト用のデータセットをいれて学習する実験  
ブロック状のノイズが生み出された。期待する結果は見られなかった。

### 現状のモデル（プログラム一式含む）でデータセットだけを変えて，実験したい
Mnistだとちいさすぎるかな？CelebAとかがやはり妥当そう．
